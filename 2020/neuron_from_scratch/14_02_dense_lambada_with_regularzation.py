...
class Layer_Dense:
  ...
  def backward(self, dvalues):
    ...
    # Gradient on regularizatoin
    # L1 on weights, see formula backward pass regularization L1
    if self.weigth_regularizer_l1 > 0:
      dL1 = np.ones(self.weights)
      dL1[self.weights < 0 ] = -1
      self.dweights += self.weigth_regularizer_l1 * dL1
    
    # L2 on weigths, see formula backward pass regularization L2
    if self.weight_regularizer_l2 > 0:
      self.dweights += 2 * self.weight_regularizer_l2 * self.weights

    # L1 on biases, see formula backward pass regularization L1
    if self.bias_regularizer_l1 > 0:
      dL1 = np.ones(self.weights)
      dL1[self.biases < 0]  = -1
      self.dbiases += self.bias_regularizer_l1 * dL1

    # L2 on biases, see formula backward pass regularization L2
